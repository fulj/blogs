...
你好，我是傅良军来自浙江金华浦江县，从事java开发已经8年多了，我现在做的项目是云上数据库管控系统，涉及java开发和python开发，主要是通过java应用提供web接口，实现python自动化运维。打个比方创建一个mysql实例或者按时间点恢复一个实例，通过restful接口下发一个任务python应用通过轮询任务列表抓去准备中的任务，通过task_key分配不同的任务流，去执行相应的运维操作
